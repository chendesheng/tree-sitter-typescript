...
pub fn build(b: *std.Build) !void {
    ...
    const library_name = "tree-sitter-typescript";
    ...
    lib.addCSourceFile(.{
        .file = b.path("typescript/src/parser.c"),
        .flags = &.{"-std=c11"},
    });
    if (fileExists(b, "typescript/src/scanner.c")) {
        lib.addCSourceFile(.{
            .file = b.path("typescript/src/scanner.c"),
            .flags = &.{"-std=c11"},
        });
    }
    lib.addCSourceFile(.{
        .file = b.path("tsx/src/parser.c"),
        .flags = &.{"-std=c11"},
    });
    if (fileExists(b, "tsx/src/scanner.c")) {
        lib.addCSourceFile(.{
            .file = b.path("tsx/src/scanner.c"),
            .flags = &.{"-std=c11"},
        });
    }
    ...
    lib.addIncludePath(b.path("typescript/src"));
    lib.addIncludePath(b.path("tsx/src"));

    b.installArtifact(lib);
    b.installFile("typescript/src/node-types.json", "node-types.json");
    b.installFile("tsx/src/node-types.json", "node-types-tsx.json");
